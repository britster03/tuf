...
public class DualRowAssignment4 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j <= target; j++) {
                ...
                // cs 1 : assigning the current block to Row 1 first
                ...
                // cs 2 : assigining the current block to Row 0
                if (j + elems[i].len <= target) {
                    ...
                    // updating the DP table and recording choice if a lower cost is found
        ...
        // the backtracking is done to determine which blocks were assigned to which row
        for (int i = n; i > 0; i--) {
            if (choice[i][j] == 0) {
                // if the block was assigned to Row 0
                r0.add(0, i); // add to Row 0 (1-based indexing - this means the rows are indexed as 1,2,3,4
                              // not 0,1,2,3)
                j -= elems[i - 1].len; // moving to the previous state by reducing the length
            } else {
                // if the block was assigned to Row 1
                r1.add(0, i); // add to Row 1 (1-based indexing)
